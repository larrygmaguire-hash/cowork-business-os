...
description: Create, edit, and analyse spreadsheets with Excel formulas and formatting. Handles .xlsx, .csv, and other common spreadsheet formats.
---
...
This skill reads `.claude/company/overview.md` for business context when interpreting data, if available. If you frequently work with specific metrics or KPIs, provide that information upfront. The skill works generically without it.
...
| **Clean** | Remove duplicates, fix formatting, standardise values |
...
- For large files (10,000+ rows): summarise rather than loading everything into context
- Always verify totals with spot checks against source data

## Formula Guidelines

- Use named ranges or structured references where possible
- Include formula comments explaining logic
- Test formulas with sample data before applying to full dataset
- Common formulas: SUMIFS, COUNTIFS, VLOOKUP, IF/IFS, INDEX/MATCH, AVERAGEIFS
...
- [ ] Data cleaned and free of obvious errors

## When NOT to Use

- For PDF data extraction — use processing-pdfs first, then this skill for analysis
- For database queries (different tooling)
- For presentation of data as slides — use presentation processing skill

## Error Handling

- **File cannot be read:** Report the error and suggest format conversion
- **Data too large for context:** Describe structure and ask which columns/rows to focus on
- **Ambiguous request:** Ask what specific metric or insight the user is looking for
- **Missing dependencies:** Provide installation instructions for required libraries
