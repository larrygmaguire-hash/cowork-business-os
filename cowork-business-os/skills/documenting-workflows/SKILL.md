...
description: Document business workflows, standard operating procedures (SOPs), process guides, checklists, and onboarding materials. Use when asked to "document this process", "write an SOP", "create a checklist", "write a procedure", "document how we do [X]", "onboarding guide", or "create a playbook".
metadata:
  version: 1.0.0
---
...
If these files contain placeholders, the skill works generically and prompts you to provide role names inline.
...
| **Runbook** | Technical operational procedure (incident response, deployment, maintenance) | 2-5 pages |
...
- Include the system or tool name in the step (e.g., "Open the invoice in Xero" not "Open the invoice")
- Write for someone doing this for the first time
- Avoid assumed knowledge — if a step requires a login, say so
- Note where screenshots or visual cues would help (mark as "[Screenshot: description]")
...
6. Save as markdown or .docx (per user preference)
...
- [ ] Compliance requirements addressed (if industry.md populated)
- [ ] Consistent terminology throughout

## When NOT to Use

- For technical code documentation (use project CLAUDE.md)
- For writing articles or marketing content (use copywriting)
- For creating the .docx file format specifically (use processing-documents)

## Error Handling

- **Vague process description:** Ask clarifying questions about triggers, steps, decision points, and end states before drafting. Do not guess at process details.
- **Missing role information:** Use generic role names (e.g., "Team Lead", "Administrator") and note that these should be replaced with actual role titles.
- **Complex process with many branches:** Break into sub-procedures, each documented separately, with cross-references between them.
