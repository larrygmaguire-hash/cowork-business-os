...
export class PubMedClient {
    ...
    async search(query, options) {
        ...
        const searchUrl = this.appendApiKey(`${BASE_URL}/esearch.fcgi?${searchParams}`);
        const searchResponse = await fetch(searchUrl);
        if (!searchResponse.ok) {
            throw new Error(`PubMed esearch failed: ${searchResponse.status} ${searchResponse.statusText}`);
        }
        ...
        const fetchUrl = this.appendApiKey(`${BASE_URL}/efetch.fcgi?${fetchParams}`);
        const fetchResponse = await fetch(fetchUrl);
        if (!fetchResponse.ok) {
            throw new Error(`PubMed efetch failed: ${fetchResponse.status} ${fetchResponse.statusText}`);
        }
    ...
    parseArticles(xmlText) {
        ...
        if (!Array.isArray(articles))
            articles = [articles];
        return articles.map((article) => this.mapToPaper(article));
    }
    mapToPaper(article) {
        ...
        // Title
        const title = String(articleData?.ArticleTitle ?? "").replace(/<[^>]*>/g, "");
        // Authors
    ...
    parseAbstract(abstractText) {
        ...
        if (abstractText["#text"])
            return String(abstractText["#text"]);
        // Array of sections
    ...
    parseYear(articleData) {
        // Try JournalIssue PubDate first
        const pubDate = articleData?.Journal?.JournalIssue?.PubDate?.Year;
        if (pubDate)
            return Number(pubDate);
        // Try ArticleDate
...
//# sourceMappingURL=pubmed-client.js.map