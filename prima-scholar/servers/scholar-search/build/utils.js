 ...
 */
function formatAuthorApa7(author) {
 ...
 */
export function formatApa7Citation(paper) {
    // Format authors
    let authorStr;
    ...
    if (paper.journal) {
        ...
        const volIssueParts = [];
        if (paper.volume)
            volIssueParts.push(paper.volume);
        if (paper.issue)
            volIssueParts.push(`(${paper.issue})`);
        if (volIssueParts.length > 0)
            citation = citation.replace(/\.$/, `, ${volIssueParts.join("")}.`);
        if (paper.pages)
            citation = citation.replace(/\.$/, `, ${paper.pages}.`);
    }
...
export function formatHarvardCitation(paper) {
    let authorStr;
    if (paper.authors.length === 0) {
        authorStr = "Unknown";
    }
    ...
    if (paper.journal) {
        citation += ` ${paper.journal}.`;
        if (paper.volume)
            citation = citation.replace(/\.$/, `, vol. ${paper.volume}.`);
        if (paper.issue)
            citation = citation.replace(/\.$/, `, no. ${paper.issue}.`);
        if (paper.pages)
            citation = citation.replace(/\.$/, `, pp. ${paper.pages}.`);
    }
    if (paper.doi) {
...
export function formatChicagoCitation(paper) {
    let authorStr;
    if (paper.authors.length === 0) {
        authorStr = "Unknown";
    }
...
export function formatVancouverCitation(paper) {
    let authorStr;
    if (paper.authors.length === 0) {
        authorStr = "Unknown";
    }
...
export function formatIeeeCitation(paper) {
    let authorStr;
    if (paper.authors.length === 0) {
        authorStr = "Unknown";
    }
...
export function formatMlaCitation(paper) {
    let authorStr;
    if (paper.authors.length === 0) {
        authorStr = "Unknown";
    }
...
export function formatAllCitations(paper) {
    const styles = ['apa7', 'harvard', 'chicago', 'vancouver', 'ieee', 'mla'];
    const citations = {};
 ...
 */
export function deduplicateByDoi(papers) {
