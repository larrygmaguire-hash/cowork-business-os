    ...
    try {
        switch (name) {
            ...
            case "scholar_search": {
                ...
                const openAccessOnly = args?.open_access_only ?? false;
                const yearFrom = args?.year_from;
                const yearTo = args?.year_to;
                const citationStyle = args?.citation_style;
                const searchOptions = {
                    maxResults,
                    yearFrom,
                    yearTo,
            ...
            case "scholar_citations": {
                ...
                if (!paperId)
                    throw new McpError(ErrorCode.InvalidParams, "paper_id is required");
...
// ── Aggregated Search ────────────────────────────────────────────────
async function aggregatedSearch(query, sources, options) {
    const activeSources = [];
    ...
    for (const source of sources) {
        ...
        if (keyInfo && !process.env[keyInfo.envVar]) {
            missingApiKeys.push(`${source}: API key not configured. Set ${keyInfo.envVar} in your environment. ` +
    ...
    if (activeSources.length === 0) {
        throw new Error(`No sources available. Missing API keys:\n${missingApiKeys.join("\n")}`);
    }
...
// ── Paper ID Routing ─────────────────────────────────────────────────
async function routeGetPaper(id) {
...
// ── Full Text Retrieval ──────────────────────────────────────────────
async function retrieveFullText(id) {
    ...
    return {
        source: "none",
        message: "Full text not available through PRIMA Scholar. " +
