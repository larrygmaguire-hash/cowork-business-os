...
const rawDbPath = process.env.RESEARCH_LIBRARY_PATH ?? "~/.research-library/library.db";
    ...
    try {
        switch (name) {
            case "library_import": {
                ...
                switch (ext) {
                    ...
                    case ".txt":
                        ...
                        break;
                    default:
                        throw new McpError(ErrorCode.InvalidParams, `Unsupported file type: ${ext}. Supported: .pdf, .docx, .md, .txt`);
                }
            ...
            case "library_get_document": {
                ...
                if (!doc) {
                    throw new McpError(ErrorCode.InvalidParams, `Document ${id} not found`);
                }
            ...
            case "library_update": {
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: JSON.stringify({ id, updated, status: updated ? "updated" : "no changes" }, null, 2),
                        },
                    ],
                };
            }
            case "library_delete": {
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: JSON.stringify({ id, deleted, status: deleted ? "deleted" : "not found" }, null, 2),
                        },
                    ],
                };
            }
            case "library_create_collection": {
                const collName = args?.name;
                if (!collName) {
                    throw new McpError(ErrorCode.InvalidParams, "name is required");
                }
                const collId = db.createCollection(collName, args?.description);
                return {
                    content: [
                        {
                            type: "text",
                            text: JSON.stringify({ id: collId, name: collName, status: "created" }, null, 2),
                        },
                    ],
                };
            }
            ...
            case "library_add_to_collection": {
                ...
                const collName = args?.collection_name;
                if (docId === undefined || !collName) {
                    throw new McpError(ErrorCode.InvalidParams, "document_id and collection_name are required");
                }
                const collection = db.getCollectionByName(collName);
                if (!collection) {
                    throw new McpError(ErrorCode.InvalidParams, `Collection "${collName}" not found`);
                }
            ...
            case "library_tag": {
                ...
                if (docId === undefined || !tagName || !action) {
                    throw new McpError(ErrorCode.InvalidParams, "document_id, tag_name, and action are required");
                }
                let success;
                if (action === "add") {
                    ...
                    throw new McpError(ErrorCode.InvalidParams, 'action must be "add" or "remove"');
                }
                return {
                    content: [
                        {
                            type: "text",
                            text: JSON.stringify({ document_id: docId, tag: tagName, action, success }, null, 2),
                        },
                    ],
                };
            }
        ...
        throw new McpError(ErrorCode.InternalError, `Library error: ${message}`);
    }
});
// Start server
async function main() {
